...
A library and command-line tool for working with data, used by a developer from
their own terminal or taken as a dependency, rather than run as a hosted
service. Which capability it offers is not settled yet - the first objective
filed here decides that, and this sentence gets rewritten then rather than left
to drift.
...
index, a container, anything that builds this repository somewhere other than
the gate - record in the same ADR what builds it, how that build differs from
what CI runs, and what covers the difference. A gate that does not run what
publishes a project is a gap nothing measures until it has been wrong for a
day.
